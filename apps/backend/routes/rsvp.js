...
router.get('/:code', lookupRateLimit, async (req, res) => {
  ...
  try {
    const result = await Guest.findByCodeWithPlusOne(code);
    if (!result) return sendNotFound(res, 'Guest', code);
    
    const { primary, plusOne } = result;
    const guestData = {
      ...primary,
    ...
    setGuestSession(res, primary.id, primary.code);
    const auth = {
      name: primary.name,
      group_label: primary.group_label,
      rsvp_status: primary.rsvp_status
    };
    return res.json({ success: true, guest: guestData, auth });
  } catch (err) {
    logger.error('[RSVP_ROUTE] Error looking up guest by code:', err);
...
router.post('/', async (req, res) => {
  ...
  try {
    ...
    // Update RSVP
    await Guest.updateRsvp(guest.id, {
      attending,
      dietary,
      notes
    });
    ...
    // Log for debugging
    logger.info(`[RSVP_ROUTE] RSVP PUBLIC UPDATE: code=${code}`, {
      attending,
      rsvp_status: updatedGuest.rsvp_status,
      plus_one_name: plus_one_name !== undefined ? plus_one_name : null,
    ...
    const guestDataForEmail = {
      ...
      dietary: dietary || guest.dietary || null,
      notes: notes || guest.notes || null,
    ...
    logger.info(`[RSVP_ROUTE] RSVP email decision: send_email=${send_email} (type: ${typeof send_email}), shouldSendEmail=${shouldSendEmail}, code=${code}`);
    if (shouldSendEmail) {
      try {
        await sendConfirmationEmail(db, guestDataForEmail);
      } catch (emailErr) {
        logger.error('[RSVP_ROUTE] Error in sendConfirmationEmail', emailErr);
    ...
    logger.error('[RSVP_ROUTE] Error in POST /rsvp handler', err);
